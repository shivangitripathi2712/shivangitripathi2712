<h1 align="center">Hi, I'm Shivangi Tripathi</h1>

<h3 align="center">
  Ph.D. Student in Computer Science at Texas State University
</h3>

<p align="center">
  <b>LLM Safety | Hallucination Detection & Correction | Misinformation Revision | Trustworthy AI</b>
</p>

<p align="center">
  <a href="https://scholar.google.com/citations?user=F0VM8BQAAAAJ&hl=en">
    <img src="https://img.shields.io/badge/Google%20Scholar-Profile-blue?style=for-the-badge&logo=google-scholar&logoColor=white" alt="Google Scholar" />
  </a>
  <a href="https://github.com/shivangitripathi2712">
    <img src="https://img.shields.io/badge/GitHub-shivangitripathi2712-black?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" />
  </a>
  <a href="https://shivangitripathi2712.github.io/">
    <img src="https://img.shields.io/badge/Website-Portfolio-purple?style=for-the-badge&logo=google-chrome&logoColor=white" alt="Website" />
  </a>
  
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/shivangi-tripathi-62581415b/">
    <img src="https://img.shields.io/badge/LinkedIn-Shivangi%20Tripathi-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
  <a href="Shivang_T_CV.pdf">
  <img src="https://img.shields.io/badge/CV-Download-red?style=for-the-badge&logo=adobeacrobatreader&logoColor=white" alt="CV" />
</a>
   <a href="https://orcid.org/0009-0007-4181-6582">
    <img src="https://img.shields.io/badge/ORCID-0009--0007--4181--6582-green?style=for-the-badge&logo=orcid&logoColor=white" alt="ORCID" />
  </a>
</p>

---

## About Me

I am a 3rd year **Ph.D. student in Computer Science at Texas State University**, working on **Large Language Model reliability, hallucination detection and correction, misinformation revision, adversarial robustness, and trustworthy AI**.

My research aims to make generative AI systems more **factually grounded, robust, and safe for real-world deployment**, especially in long-form generation and security-sensitive settings.

---

## Research Focus

- **Large Language Models (LLMs)**
- **Hallucination Detection and Mitigation**
- **Paragraph-Level Misinformation Correction**
- **Adversarial Attacks and Defenses in LLMs**
- **Trustworthy and Safe AI**
- **NLP and Machine Learning**

---

## Featured Research Projects

### [Long-Output Misinformation Revision](https://github.com/shivangitripathi2712/long_output_misinformation_revision)
Evidence-grounded sequential revision framework for correcting **paragraph-level misinformation** in LLM-generated text.  
This project focuses on **sentence-level verification, evidence retrieval, agreement gating, and sequential editing** to improve factual consistency while preserving the original response as much as possible.

### [Hallucination under Adversarial Attack](https://github.com/shivangitripathi2712/hallucination_under_adversarial_attack)
Research implementation for studying how **adversarial prompt perturbations** can trigger hallucinated behavior in LLMs.  
This project explores **OoD prompts, weak semantic perturbations, and adversarial prompt optimization** to analyze model vulnerability and robustness.

---

## Selected Publications

### 1. REVISE: A Framework for Paragraph-Level Misinformation Correction in Large Language Models  
**Shivangi Tripathi, Teancy Jennifer, Henry Griffith, Heena Rathore**  
*2025 IEEE International Conference on Artificial Intelligence Testing (AITest)*

### 2. Detecting and Correcting Hallucinations in Paragraph-Level Text with Ensemble-Based Evaluation  
**Shivangi Tripathi, Heena Rathore**  
*2025 IEEE 7th International Conference on Trust, Privacy and Security in Intelligent Systems and Applications (TPS-ISA)*

### 3. Paragraph-Level Hallucination Detection and Correction for Trustworthy Large Language Models in Networked Systems  
**Shivangi Tripathi, Teancy Jennifer, Henry Griffith, Heena Rathore**  
*2026 IEEE 23rd Consumer Communications & Networking Conference (CCNC)*

---

## Current Interests

I am particularly interested in building frameworks for:

- **Hallucination detection and correction in long-form LLM outputs**
- **Reliable evidence-grounded revision systems**
- **LLM safety under adversarial settings**
- **Trustworthy generative AI for real-world applications**

---

## Connect With Me

- **Google Scholar:** [View Profile](https://scholar.google.com/citations?user=F0VM8BQAAAAJ&hl=en)
- **ORCID:** [0009-0007-4181-6582](https://orcid.org/0009-0007-4181-6582)

---

<p align="center">
  <i>Researching reliable, safe, and trustworthy large language models.</i>
</p>
